...
const PropertyDataVisualization: React.FC = () => {
  ...
  const getComposedAnalysis = () => {
    const filteredData = data.filter(
      (p) => p.actual_price >= priceFilter.min && p.actual_price <= priceFilter.max
    );
    ...
    return Object.values(analysis)
      .map(({ range, prices, ratings, counts }) => ({
        range,
  ...
  const getAreaTrends = () => {
    ...
    const yearGroups = sorted.reduce(
      (acc, property) => {
      ...
      {} as Record<
        number,
        { year: number; prices: number[]; ratings: number[]; walkability: number[] }
      >
    );

    return Object.values(yearGroups)
      .map(({ year, prices, ratings, walkability }) => ({
        year: year.toString(),
