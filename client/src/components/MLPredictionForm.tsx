  ...
  return (
    <div className="ml-prediction-form">
      <div className="form-container">
        <h2>
          <span aria-hidden="true">🏠</span> Property Price Prediction
        </h2>

        <form
          onSubmit={handleSubmit}
          className="prediction-form"
          aria-label="Property price prediction form"
        >
          ...
          <button type="submit" className="predict-button" disabled={loading} aria-busy={loading}>
            {loading ? (
              <>
                <span aria-hidden="true">🔄</span> Predicting...
              </>
            ) : (
              <>
                <span aria-hidden="true">🎯</span> Get Price Prediction
              </>
            )}
          </button>
        </form>
        ...
        {error && (
          <div className="error-message" role="alert" aria-live="assertive">
            ...
            <button onClick={clearError} className="close-error" aria-label="Close error message">
              ×
            </button>
          </div>
        )}
      ...
      {result && (
        ...
        >
          <h3>
            <span aria-hidden="true">📊</span> Prediction Results
          </h3>
          <div className="results-grid">
            <div className="result-card main-prediction">
              <h4>
                <span aria-hidden="true">💰</span> Estimated Price
              </h4>
              <div
                className="price-estimate"
                aria-label={`Estimated price: ${formatCurrency(result.prediction.prediction.point_estimate)}`}
              >
                {formatCurrency(result.prediction.prediction.point_estimate)}
              </div>
              <div className="confidence-interval">
                Confidence Interval (95%):{' '}
                {formatCurrency(result.prediction.prediction.lower_bound)} -{' '}
                {formatCurrency(result.prediction.prediction.upper_bound)}
              </div>
            </div>

            <div className="result-card">
              <h4>
                <span aria-hidden="true">🤖</span> Model Used
              </h4>
            ...
            <div className="result-card">
              <h4>
                <span aria-hidden="true">⏱️</span> Execution Time
              </h4>
            ...
            <div className="result-card">
              <h4>
                <span aria-hidden="true">📈</span> Confidence Level
              </h4>
