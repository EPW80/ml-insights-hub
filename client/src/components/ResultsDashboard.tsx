...
const ResultsDashboard: React.FC = () => {
  ...
  const [activeMetric, setActiveMetric] = useState<
    'predictions' | 'accuracy' | 'models' | 'performance'
  >('predictions');
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    if (!hasPredictions) {
      allPredictions = Array.from({ length: 20 }, (_, i) => ({
        ...
        modelType: ['random_forest', 'linear_regression', 'neural_network'][
  ...
  const getModelUsageData = () => {
    ...
    const usage = dashboardData.predictions.reduce(
      (acc, pred) => {
        ...
        <div className="summary-card">
          ...
          <div className="card-content">
            <div className="card-value">
              {(dashboardData.summary.avgConfidence * 100).toFixed(1)}%
            </div>
          ...
          <div className="table-body">
            {dashboardData.predictions.slice(0, 8).map((pred) => (
              <div key={pred.id} className="table-row">
                <div className="table-cell">{formatTimestamp(pred.timestamp)}</div>
                <div className="table-cell">
                  <span className="model-badge">{pred.modelType.replace('_', ' ')}</span>
                </div>
                <div className="table-cell prediction-value">{formatCurrency(pred.prediction)}</div>
                <div className="table-cell">
                ...
                <div className="table-cell">
                  {pred.features.bedrooms}BR/{pred.features.bathrooms}BA,{' '}
                  {pred.features.sqft?.toLocaleString()}sq ft
                </div>
              </div>
            ))}
          </div>
