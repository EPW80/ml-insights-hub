...
class MLWebSocketManager {
  ...
  setupHeartbeat(socket, connectionInfo) {
    const heartbeatTimer = setInterval(() => {
      if (connectionInfo.isAlive) {
        ...
        logger.warn(`Heartbeat failed for ${socket.id}, terminating connection`);
        socket.terminate();
  ...
  async handleTrainingRequest(socket, connectionInfo, config, jobId) {
    ...
    if (!modelType || !datasetId) {
      throw new Error('Missing required training parameters: modelType and datasetId');
    }
  ...
  async handlePredictionRequest(socket, connectionInfo, config) {
    ...
    if (!features || !modelType) {
      throw new Error('Missing required prediction parameters: features and modelType');
    }
  ...
  async handleAnalysisRequest(socket, connectionInfo, config) {
    ...
    if (!datasetId || !analysisType) {
      throw new Error('Missing required analysis parameters: datasetId and analysisType');
    }
  ...
  handleDisconnection(connectionId, reason) {
    logger.info(`Client disconnected: ${connectionId}, reason: ${reason}`);
